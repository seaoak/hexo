...
function Box(ctx, base, options) {
  ...
  if (ctx.config.ignored_path !== undefined) {
    this.options.ignored = convertIgnoredPathConfig(ctx, ctx.config.ignored_path);
    this.options.ignored.push(/[\/\\](\.git|hoge)ZZZ[\/\\]?$/i); // seaoak
    this.options.ignored.push(/^foobar\.txt$/); // seaoak
    console.log('seaoak options.ignored = ' + this.options.ignored.toString());
  }
...
function convertIgnoredPathConfig(ctx, arg) {
  function toRegExp(entry) {
    if (typeof entry === 'object' && entry === null) return;
    if (typeof entry !== 'string') {
      console.log('seaoak toRegExp: (' + (typeof entry) + ') = ' + JSON.stringify(entry));
      ctx.log.warn('A value of "ignored_path:" setting in the site config file is not a string. So skipped. The value is: ' + entry);
      return;
    }

    if (!entry) return;

    var result = minimatch.makeRe(entry, {});
    if (!result) {
      console.log('seaoak toRegExp: ' + entry);
      ctx.log.warn('A value of "ignored_path:" setting in the site config file can not be interpreted. So skipped. The value is: ' + entry);
      return;
    }
  ...
  console.log('seaoak convertIgnoredPathConfig: ' + JSON.stringify(arg));

  var arr = Array.isArray(arg) ? arg.map(toRegExp) : [toRegExp(arg)];
  return arr.filter(_.identity);
}
...
Box.prototype._readDir = function(base, fn, prefix) {
  ...
  if (self._isIgnorablePath(base)) console.log('seaoak ignored: ' + base);
  if (self._isIgnorablePath(base)) return [];

  return fs.readdir(base).map(function(path) {
    if (self._isIgnorablePath(join(base, path))) console.log('seaoak ignored: ' + join(base, path));
    if (self._isIgnorablePath(join(base, path))) return [];
...
Box.prototype._isIgnorablePath = function(path) {
  var patterns = this.options.ignored;
...
Box.prototype._processFile = function(type, path) {
  ...
  console.log('seaoak Box.prototype._processFile : ' + path);
  return Promise.reduce(this.processors, function(count, processor) {
    ...
    console.log('seaoak process done: ' + path);
  }).thenReturn(path);
};
