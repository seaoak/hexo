...
function convertIgnoredPathConfig(ctx, arg) {
  function toRegExp(entry) {
    ...
    if (typeof entry !== 'string') {
      ...
      ctx.log.warn('The value of "ignored_path" parameter in the site config file should be string(s). So skipped.');
      return;
    }

    if (!entry) return;
    var isLikeRegExp = entry.match(/^\/(.*)\/([gimuy]*)$/);
    if (isLikeRegExp) return new RegExp(isLikeRegExp[1], isLikeRegExp[2].replace(/[gmy]/, ''));
    if (pathFn.isAbsolute(entry)) return new RegExp('^' + _.escapeRegExp(entry) + '[\\/\\\\]?$');
    return new RegExp('(^|[\\/\\\\])' + _.escapeRegExp(entry) + '[\\/\\\\]?$');
  }
